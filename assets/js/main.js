...
$(document).ready(function () {
    ...
    $('.filter-control').on('click', 'li', function () {
        $('.filter-control li').removeClass('active');
        $(this).addClass('active');
        callProductsAPI($(this).attr('data-id'));
    })

    function callProductsAPI(categoryId) {
        let url = "https://ashion-api.onrender.com/products?_page=1&_limit=8"
        if (categoryId) {
    ...
    function callCategoriesAPI() {
        $.ajax({
            url: "https://ashion-api.onrender.com/categories?_page=1&_limit=5",
            method : "GET",
            success: function (data) {
                renderCategori(data);
                renderFilterCategori(data);
            },
            error: function (error) {
    ...
    let leftHtml = `
        <div class="col-lg-6">
            <div class="hero-item hero-item-large">
                <img src="${catLarge.image || './assets/img/default.jpg'}" alt="${catLarge.name}" class="bg">
                <div class="hero-text">
                    <h1>${catLarge.name}</h1>
        ...
        rightHtml += `
            <div class="col-lg-6 col-md-6 col-sm-6">
                <div class="hero-item set-bg">
                    <img src="${catSmall.image || './assets/img/default.jpg'}" alt="${catSmall.name}" class="bg">
                    <div class="hero-text">
                        <h4>${catSmall.name}</h4>
...
function renderFilterCategori(categori) {
    let html = `<li class='active'>All</li>`;
    categori.forEach(function (item) {
        html += `<li data-id="${item.id}">${item.name}</li>`;
    })
    $('#filter-nav').html(html);
}
...
function renderProductsHTML(productsList) {
    ...
    if (!productsList || productsList.length === 0) {
        html = `<p>Danh mục này ko có sản phẩm nào cả</p>`
    ...
    productsList.forEach(function (product) {
        ...
        let totalStock = product.variants.reduce(function (sum, item) {
        ...
        let cartButtonHtml = totalStock > 0
            ? `<li><a href="./shop-cart.html" class="add-to-cart-btn"><span class="icon_bag_alt"></span></a></li>`
            : ``;

        let mainImage = './assets/img/default-placeholder.jpg';
        if (product.images && product.images.length > 0) {
            mainImage = product.images[0].big || product.images[0];
        }
